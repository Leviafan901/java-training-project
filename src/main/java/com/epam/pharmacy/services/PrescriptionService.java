...
public class PrescriptionService {
	...
	private static final int PROLONGATION_TERM = 1;
	...
	/***
	 * Method that change status of Request row to 'approved' and change value of expiration_date in the Prescription table
	 * 
	...
	public boolean extendPrescription(Long prescriptionId, Long requestId, LocalDate expirationDate) throws ServiceException {
		try (DaoCreator daoCreator = new DaoCreator()) {
				PrescriptionDao prescriptionDao = daoCreator.getPrescriptionDao();
				LocalDate actualExpirationDate = expirationDate.plusMonths(PROLONGATION_TERM);
