...
<fmt:bundle basename="i18n">
	...
	<fmt:message key="medicines.count_in_store" var="countInStore" />
	<fmt:message key="orders.client" var="client" />
	<fmt:message key="orders.cancel" var="cancel" />
	<fmt:message key="orders.mark_as_paid" var="markPaid" />
</fmt:bundle>
				...
				<ul class="drugsForm__items">
					<li class="drugsForm__item sku-holder" data-sku-id="60591"
						...
						<div class="drugsForm__item-left">
							<div class="drugsForm__item-description">
								<div>
									<span>${client}: ${order.name} ${order.lastname}</span>
								</div>
								<div>
									<span>${order_date}: ${order.date}</span>
								</div>
								...
								<div>
									<span>${countInStore}: ${order.countInStore}</span>
								</div>
								<div>
							...
							<c:choose>
								<c:when
									test="${(order.status == 'IN_PROCESS') and (order.count <= order.countInStore) and (order.count > 0)}">
